...
import { type GdpPdfModelScoreRow, getGdpPdfStats } from "../scrapers/surge/gdp-pdf";

export function readGdpPdfRawCache(cache: CacheRowSource): {
  rows: GdpPdfModelScoreRow[];
  fetchedAt: number | null;
} | null {
  const cacheRows = sourceCacheRows(cache, "SELECT * FROM gdp_pdf_raw_rows ORDER BY row_index");
...
function insertGdpPdfRawRows(db: DatabaseWriter, snapshots: SourceSnapshots): void {
