...
function taskMetric(task: string): HarveyLabMetric {
  return task === CRITERION_PASS_TASK_KEY || task.startsWith(`${CRITERION_PASS_TASK_KEY}__`)
    ? "criterion_pass"
    : "task_resolution";
}
...
function harveyLabTaskRow(
  ...
  const reasoningEffort = canonicalReasoningEffort(row.reasoning_effort ?? row.compute_effort);
  return {
    task,
...
export function processHarveyLabPageHtml(pageHtml: string): HarveyLabParsedPage {
  ...
  const metadata =
    Object.keys(metadataRecord).length === 0 ? null : harveyLabMetadata(metadataRecord);
  ...
  for (const [task, taskValue] of Object.entries(asRecord(view.tasks))) {
    for (const [modelId, rowValue] of Object.entries(asRecord(taskValue))) {
      const row = harveyLabTaskRow(task, taskLabels[task] ?? task, modelId, rowValue);
      if (row != null) {
        taskRows.push(row);
      }
    }
  ...
  const overallByModelId = new Map(rankedOverallRows.map((row) => [row.model_id, row]));
  const enrichedTaskRows = taskRows.map((row) =>
    row.task === OVERALL_TASK_KEY ? (overallByModelId.get(row.model_id) ?? row) : row,
  );
  return {
    metadata,
