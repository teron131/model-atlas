...
export type DeepSWESourceVersion = typeof DEEP_SWE_PREFERRED_SOURCE_VERSION | "v1";
...
function asDeepSWELeaderboardRow(value: unknown): DeepSWELeaderboardRow | null {
  ...
  return {
    ...
    config: typeof row.config === "string" && row.config.length > 0 ? row.config : null,
    pass_at_1: passAt1,
...
export function asDeepSWERawLeaderboardRow(value: unknown): DeepSWERawLeaderboardRow | null {
  const row = asRecord(value);
  ...
  return {
    ...leaderboardRow,
    source_version:
      row.source_version === "v1.1" || row.source_version === "v1" ? row.source_version : null,
  };
}
...
function stripDeepSWESourceVersion(row: DeepSWERawLeaderboardRow): DeepSWELeaderboardRow {
  return {
    model: row.model,
...
export function preferredDeepSWELeaderboardRows(
  ...
  const v1OnlyRows = rows.filter(
    (row) => row.source_version !== "v1.1" && !v11RowKeys.has(deepSwePreferenceKey(row)),
  );
...
function deepSwePreferenceKey(row: DeepSWERawLeaderboardRow): string {
  return [normalizeModelToken(row.model), row.reasoning_effort ?? "", row.config ?? ""].join("\0");
}
...
export async function getDeepSWERawLeaderboardSourceRows(
  ...
  const urls = options.url != null ? [options.url] : (options.urls ?? DEFAULT_LEADERBOARD_URLS);
...
export function deepSWEUrlForSourceVersion(version: DeepSWESourceVersion | null): string {
  return version === "v1" ? DEEP_SWE_V1_LEADERBOARD_URL : DEEP_SWE_V1_1_LEADERBOARD_URL;
}
