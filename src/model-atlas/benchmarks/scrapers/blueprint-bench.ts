...
export type BlueprintBenchRowsByModelName = Map<string, BlueprintBenchModelScoreRow>;
...
function parseScore(value: string | undefined): number | null {
  ...
  return Number.isFinite(score) && score >= 0 && score <= 1 ? Number(score.toFixed(6)) : null;
}
...
export function processBlueprintBenchPageHtml(pageHtml: string): BlueprintBenchModelScoreRow[] {
  ...
  const leaderboardLines = end === -1 ? lines.slice(bodyStart) : lines.slice(bodyStart, end);
...
export function findBlueprintBenchScore(
  ...
  for (const candidateName of candidateNames) {
    ...
    const row = blueprintBenchRowsByModelName.get(normalizeModelToken(candidateName));
    if (row) {
      return row.score;
    }
  }
