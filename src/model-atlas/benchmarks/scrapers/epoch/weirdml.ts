...
export const WEIRDML_EPOCH_CSV_URL = "https://epoch.ai/data/external_benchmarks/weirdml.csv";
...
function epochModelEffort(displayName: string, modelVersion: string) {
  ...
  return {
    ...
    reasoningEffort:
      suffix.toLowerCase() === "no_thinking" ? "none" : canonicalReasoningEffort(suffix),
  };
}
    ...
    return [
      {
        ...
        name,
        aliases: [
          ...new Set([modelVersion, ...(displayAliases.length > 0 ? displayAliases : [name])]),
        ],
