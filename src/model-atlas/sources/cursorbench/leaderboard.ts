...
export const DEFAULT_LEADERBOARD_URL = "https://cursor.com/cursorbench";

const DEFAULT_TIMEOUT_MS = 30_000;

const LEADERBOARD_HEADER = "Model Score Cost Cost / task Tokens Tokens / task Steps Steps / task";
...
export function processCursorBenchPageHtml(pageHtml: string): CursorBenchModelScoreRow[] {
...
export function findCursorBenchScore(
  ...
  for (const candidateName of candidateNames) {
    ...
    const row = cursorBenchRowsByModelName.get(normalizeModelToken(candidateName));
    if (row) {
      return row.score;
    }
  }
...
function findLeaderboardBodyStart(lines: string[]): number {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    const possibleHeader = lines.slice(index, index + LEADERBOARD_HEADER_CELLS.length);
    if (
      possibleHeader.length === LEADERBOARD_HEADER_CELLS.length &&
      possibleHeader.every((value, headerIndex) => value === LEADERBOARD_HEADER_CELLS[headerIndex])
    ) {
...
function parseCompactCursorBenchRow(line: string): CursorBenchModelScoreRow | null {
...
function parseCursorBenchCells(lines: string[], index: number): ParsedCursorBenchCellRow | null {
