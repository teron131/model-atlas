...
import {
  ...
  sqliteBooleanValue,
  type SqlValue,
} from "../../database/writers/database";
...
import { artificialAnalysisModelId } from "./leaderboard";
import {
...
export function insertArtificialAnalysisRawModels(
  ...
  const selectedRows = selectedRowsByKey(snapshots.artificialAnalysisSelectedRows);
  const statement = db.prepare(`
		INSERT INTO artificial_analysis_raw_models (
			...
			apex_agents, critpt, gdpval_normalized, gpqa, hle,
...
export function insertArtificialAnalysisBenchmarkResourceRawRows(
  ...
  const statement = db.prepare(`
	...
	`);
  for (const [index, row] of snapshots.artificialAnalysisBenchmarkResourceRows.entries()) {
    statement.run(
      index,
...
function selectedRowsByKey(rows: readonly JsonObject[]): Map<string, JsonObject> {
...
function identityValues(row: JsonObject, selectedRow: JsonObject, creator: JsonObject): SqlValue[] {
  const modelId =
  ...
  return [
    ...
    absoluteArtificialAnalysisUrl(
      firstString(selectedRow, ["model_url"]) ?? firstString(row, ["model_url"]),
    ),
...
function benchmarkValues(row: JsonObject, selectedRow: JsonObject): SqlValue[] {
  ...
  return [
    ...
    firstNumber(row, ["median_time_to_first_chunk", "medianTimeToFirstTokenSeconds"]),
    firstNumber(selectedRow, ["median_end_to_end_response_time"]) ??
      firstNumber(row, ["median_end_to_end_response_time", "medianEndToEndResponseTimeSeconds"]),
