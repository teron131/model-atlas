...
import { artificialAnalysisCacheHasHiddenRows } from "../artificial-analysis/cache";
import { DEEP_SWE_PREFERRED_SOURCE_VERSION } from "../deep-swe/leaderboard";
import { openRouterCacheHasCurrentShape } from "../openrouter/cache";
import {
...
export function readRawSourceCacheStatus(
  ...
  const sourcePredicate = sharedObservationSource ? " WHERE source_key = ?" : "";
  const statement = db.prepare(
    `SELECT COUNT(*) AS row_count, MAX(fetched_at_epoch_seconds) AS last_fetch_epoch_seconds FROM ${table}${sourcePredicate}`,
  );
  const row = asRecord(sharedObservationSource ? statement.get(source) : statement.get());
...
export function rawSourceCacheStatusFromRows(
  ...
  nowEpochSeconds: number,
  persistedStatus?: Pick<RawSourceCacheStatus, "last_fetch_epoch_seconds" | "source_input_count">,
): RawSourceCacheStatus {
  const lastFetch =
    persistedStatus == null ? firstEpochSecond(rows) : persistedStatus.last_fetch_epoch_seconds;
...
function isSourceCacheShapeCurrent(db: DatabaseSync, source: RawSourceName): boolean {
...
function hasPreferredDeepSWECacheVersion(db: DatabaseSync): boolean {
  return (
    db
      .prepare("SELECT 1 FROM deep_swe_raw_rows WHERE source_version = ? LIMIT 1")
