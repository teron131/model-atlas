/** FrontierCode runtime owns raw-cache reconstruction, snapshot refresh, and raw-row serialization. */
...
export const frontierCodeRuntime = defineBenchmarkRuntime({
...
export function readFrontierCodeRawCache(cache: CacheRowSource): {
  ...
  if (
    ...
    cacheRows.some(
      (row) =>
        stringValue(row.url) !== FRONTIER_CODE_LEADERBOARD_URL ||
  ...
  const rows = cacheRows.flatMap<FrontierCodeModelEffortRow>((row) => {
    ...
    const extended = processFrontierCodeSubsetMetrics(jsonValue(row.extended_json));
    if (
      model == null ||
      baseModel == null ||
      sourceEffort == null ||
  ...
  return rows.length !== cacheRows.length ? null : { rows, fetchedAt: firstEpochSecond(cacheRows) };
}
...
function buildFrontierCodeStoredSubset(metrics: FrontierCodeSubsetMetrics) {
  return {
...
function insertFrontierCodeRawRows(db: DatabaseWriter, snapshots: SourceSnapshots): void {
  ...
  for (const [index, row] of snapshots.frontierCodeRows.entries()) {
    statement.run(
      ...
      JSON.stringify(buildFrontierCodeStoredSubset(row.main)),
      JSON.stringify(buildFrontierCodeStoredSubset(row.extended)),
    );
  }
}
