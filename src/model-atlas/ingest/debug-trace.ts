...
function modelsDevRowIndexByKey(snapshots: SourceSnapshots): Map<string, number> {
  const byKey = new Map<string, number>();
  let rowIndex = 0;
  for (const [providerId, provider] of Object.entries(snapshots.modelsDevPayload)) {
...
export function buildDebugTraceRows(
  ...
  const openRouterStatsRowIndexes = openRouterStatsRowIndexById(openRouterRawPayload);
  const rows: DebugTraceRow[] = [];

  for (const model of diagnostics.models) {
    const selectedModelId = model.best_match?.model_id ?? null;
    const matchSlug = artificialAnalysisMatchSlug(model.artificial_analysis_slug);
    ...
    for (const [candidateIndex, candidate] of model.candidates.entries()) {
      ...
      const variantRejected = hasVariantConflict(matchSlug, candidate.model_id, matcherConfig);
      ...
      rows.push({
        ...
        selected: isSelected,
        rejection_reason: debugRejectionReason(isSelected, variantRejected, selectedModelId),
        selected_model_id: selectedModelId,
        models_dev_row_index:
          modelsDevRowByKey.get(`${candidate.provider_id}|${candidate.model_id}`) ?? null,
...
export function insertDebugTraceRows(db: DatabaseWriter, rows: readonly DebugTraceRow[]): void {
