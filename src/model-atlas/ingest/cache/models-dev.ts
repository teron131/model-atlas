...
import type { ModelsDevModelRecord, ModelsDevPayload } from "../../scrapers/models-dev";
import {
  assignIfBoolean,
  assignIfNumber,
  assignIfString,
...
function modelLimit(row: CacheDbRow): ModelsDevModelRecord["limit"] | undefined {
...
function modelModalities(row: CacheDbRow): ModelsDevModelRecord["modalities"] | undefined {
  const input = modalityList(row, "input_modality", ["text", "image", "audio", "video", "pdf"]);
  const output = modalityList(row, "output_modality", ["text", "image", "audio", "video"]);
...
export function readModelsDevRawCache(db: DatabaseSync) {
  return modelsDevRawCacheFromRows(
    queryCacheRows(db, "SELECT * FROM models_dev_raw_models ORDER BY row_index"),
  );
}
