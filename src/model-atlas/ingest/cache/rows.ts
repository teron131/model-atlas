...
export function sourceCacheRows(cache: CacheRowSource, sql: string): CacheDbRow[] {
...
export function firstEpochSecond(rowsToScan: readonly CacheDbRow[]): number | null {
...
export function assignIfString(target: JsonObject, key: string, value: unknown): void {
...
export function assignIfNumber(target: JsonObject, key: string, value: unknown): void {
...
export function assignIfBoolean(target: JsonObject, key: string, value: unknown): void {
...
export function modalityList(row: CacheDbRow, prefix: string, names: string[]): string[] {
  return names.filter((name) => booleanFromSql(row[`${prefix}_${name}`]) === true);
}
