/** Harvey LAB runtime owns raw-cache reconstruction, snapshot refresh, and raw-row serialization. */
...
function harveyLabMetric(value: unknown): HarveyLabMetric | null {
  return value === "criterion_pass" || value === "task_resolution" ? value : null;
}

/** Reconstruct Harvey LAB rows without losing scoring configuration or resource fields. */
function readHarveyLabRawCache(cache: CacheRowSource): {
  ...
  if (
    cacheRows.length === 0 ||
    cacheRows.some((row) => stringValue(row.url) !== SOURCE_URLS.vals_harvey_lab)
  ) {
    return null;
  }
...
function insertHarveyLabRawRows(db: DatabaseWriter, snapshots: SourceSnapshots): void {
