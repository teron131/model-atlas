/** Mercor APEX Agents runtime owns raw-cache reconstruction, snapshot refresh, and raw-row serialization. */
...
export function readMercorApexAgentsRawCache(cache: CacheRowSource): {
  ...
  if (
    cacheRows.length === 0 ||
    cacheRows.some((row) => stringValue(row.url) !== SOURCE_URLS.mercor_apex_agents)
  ) {
    return null;
  }
...
export const mercorApexAgentsRuntime = {
