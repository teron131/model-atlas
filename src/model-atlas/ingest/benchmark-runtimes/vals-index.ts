/** Vals Index runtime owns raw-cache reconstruction, snapshot refresh, and raw-row serialization. */
...
import { snapshotRows, snapshotRowsWithStates, sourceKey } from "../source-snapshots/policy";
import { shouldUseFetchedRows, snapshotFetchedAt } from "../source-snapshots/row-snapshot";
import type {
...
function readValsIndexRawCache(cache: CacheRowSource): {
  rows: ValsIndexTaskScoreRow[];
  modelScores: ValsIndexModelScoreRow[];
  fetchedAt: number | null;
} | null {
  const cacheRows = sourceCacheRows(cache, "SELECT * FROM vals_index_raw_rows ORDER BY row_index");
  if (cacheRows.length === 0) {
    return null;
  }
  if (cacheRows.some((row) => stringValue(row.url) !== SOURCE_URLS.vals_index)) {
    return null;
  }
  const cachedRows = cacheRows.flatMap((row) => {
    ...
    return task != null && taskLabel != null && modelId != null && model != null && score != null
      ? [
          {
            task,
  ...
  return {
    rows: cachedRows,
    modelScores: cachedRows.filter((row): row is ValsIndexModelScoreRow => row.task === "overall"),
...
async function valsIndexSnapshot(
  ...
  const modelScores = rows.filter((row): row is ValsIndexModelScoreRow => row.task === "overall");
...
function insertValsIndexRawRows(db: DatabaseWriter, snapshots: SourceSnapshots): void {
