/** Agents' Last Exam runtime owns raw-cache reconstruction, snapshot refresh, and raw-row serialization. */
...
import { snapshotRowsWithStates, sourceKey } from "../source-snapshots/policy";
...
function readAgentsLastExamRawCache(cache: CacheRowSource): {
...
async function agentsLastExamSnapshot(
  ...
  if (status.cache_hit && cached != null && options.replaceSourceRows !== true) {
    const cachedSnapshot = snapshotRowsWithStates({
      ...
      options,
      rowKey: (row) => sourceKey(row.split, row.harness, row.model, row.harness_variant),
    ...
    return {
      ...
      agentsLastExamModelScores: summarizeAgentsLastExamModelScores(cachedSnapshot.rows),
      sourceStatus: {
        source: "agents_last_exam",
  ...
  return {
    ...
    agentsLastExamModelScores: summarizeAgentsLastExamModelScores(snapshot.rows),
    sourceStatus: {
      source: "agents_last_exam",
      fetchedAt,
...
function insertAgentsLastExamRawRows(db: DatabaseWriter, snapshots: SourceSnapshots): void {
...
export const agentsLastExamRuntime = {
