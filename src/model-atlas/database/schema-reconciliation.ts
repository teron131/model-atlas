...
function catalogTableShapes(
  ...
  return new Map(
    rows.flatMap((row) => {
      if (row.type !== "table" || typeof row.name !== "string" || typeof row.sql !== "string") {
        return [];
      }
...
function catalogTableSql(rows: readonly SchemaCatalogRow[]): Map<string, string> {
  return new Map(
    rows.flatMap((row) =>
      row.type === "table" && typeof row.name === "string" && typeof row.sql === "string"
        ? [[row.name, row.sql]]
        : [],
    ),
  );
...
function catalogIndexSql(rows: readonly SchemaCatalogRow[]): Map<string, string> {
  return new Map(
    rows.flatMap((row) =>
      row.type === "index" && typeof row.name === "string" && typeof row.sql === "string"
        ? [[row.name, row.sql]]
        : [],
    ),
  );
...
export function catalogTableMatchesSchema(
  ...
  return (
    currentSql != null &&
    expectedBody != null &&
    normalizeSchemaSql(currentSql) === normalizeSchemaSql(createTableSql(table, expectedBody))
  );
}
...
export function schemaTableShapes(schemaSql: string): Map<string, Map<string, SchemaColumnShape>> {
...
export function schemaTableColumns(schemaSql: string): Map<string, [string, string][]> {
  return new Map(
    [...schemaTableBodies(schemaSql)].map(([table, body]) => [table, schemaColumnEntries(body)]),
  );
}
...
function schemaIndexDefinitions(schemaSql: string): Map<string, SchemaIndexDefinition> {
  return new Map(
    schemaStatements(schemaSql).flatMap((statement) => {
      ...
      const name = match?.[1];
      const table = match?.[2];
      return name != null && table != null ? [[name, { table, sql: statement }]] : [];
    }),
  );
}
...
function primaryKeyColumnNames(columns: Map<string, SchemaColumnShape>): string[] {
  return [...columns]
