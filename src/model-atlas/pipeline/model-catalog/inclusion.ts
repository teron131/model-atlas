...
function normalizedRowProvider(row: Record<string, unknown>): string | null {
  ...
  const provider =
    normalizedId?.split("/")[0] ?? (typeof row.provider_id === "string" ? row.provider_id : null);
...
function rowText(row: Record<string, unknown>, keys: readonly string[]): string {
  return keys
    .map((key) => row[key])
...
function hasObviousImageModelLabel(row: Record<string, unknown>): boolean {
  return rowText(row, ["id", "openrouter_id", "name", "family"]).includes("image");
}
...
function isFastAliasRow(row: Record<string, unknown>): boolean {
  const normalizedId = normalizedRowId(row);
  return normalizedId != null && normalizedId.endsWith("-fast");
}
...
function modelsDevCatalogRow(modelsDevModel: ModelsDevFlatModel): Record<string, unknown> | null {
...
export function buildModelCatalogRows(
  ...
  const modelsDevCandidateRows = sourceData.modelsDev.rows
    ...
    .sort((left, right) => aliasInclusionRank(left) - aliasInclusionRank(right));
  for (const row of modelsDevCandidateRows) {
    const normalizedId = normalizedRowId(row);
    const latestFamilyKey = isLatestAliasRow(row) ? catalogFamilyKey(row) : null;
    if (
      normalizedId == null ||
      !isTextModelCatalogRow(row) ||
      representedModelIds.has(normalizedId) ||
      (latestFamilyKey != null && representedConcreteFamilies.has(latestFamilyKey))
    ) {
      continue;
    }
    trackRepresentedRow(row);
