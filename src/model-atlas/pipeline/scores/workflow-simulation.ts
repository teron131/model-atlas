...
function profileSeconds(
  ...
  return (
    profile.calls *
    (latencySeconds + inputTokens * inputTokenSeconds + outputTokens / throughputTokensPerSecond)
  );
}
...
function profileCost(
  ...
  if (!useCache || profile.calls <= 1) {
    return (profile.calls * (inputTokens * inputPrice + outputTokens * outputPrice)) / 1_000_000;
  }
  ...
  const laterInputPrice =
    cachedInputFraction * cacheReadPrice + (1 - cachedInputFraction) * inputPrice;
  const firstCallInputCost = inputTokens * inputPrice;
  const laterCallInputCost = (profile.calls - 1) * inputTokens * laterInputPrice;
...
function profileQualityMultiplier(
  ...
  const intelligenceScore = asFiniteNumber(model.component_scores?.intelligence_score);
  ...
  return qualityScore == null ? null : smoothstep(qualityScore / profile.full_credit_quality_score);
}
...
export function simulatedBlendSeconds(
  ...
  const throughputTokensPerSecond = positiveFiniteNumber(speed.throughput_tokens_per_second_median);
  const latencySeconds = asFiniteNumber(speed.latency_seconds_median);
  if (throughputTokensPerSecond == null || latencySeconds == null || latencySeconds < 0) {
    return null;
  }
  const inputTokenSeconds =
    asFiniteNumber(scoringConfig.secondsPerInputToken) ?? DEFAULT_INPUT_TOKEN_SECONDS;
  return weightedProfileMean(Object.values(scoringConfig.simulationProfiles), (profile) =>
    profileSeconds(
      profile,
      latencySeconds,
...
export function workflowPriceEfficiencySignal(
  ...
  return weightedProfileMean(Object.values(scoringConfig.simulationProfiles), (profile) => {
    ...
    return logCost == null || multiplier == null ? null : multiplier / logCost;
  });
}
