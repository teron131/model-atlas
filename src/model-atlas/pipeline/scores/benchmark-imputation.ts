...
function buildMercorApexImputation(
	...
	const crosswalk = buildAdditiveSourceCrosswalk(models, {
		...
		fallbackValue: (model) =>
			asFiniteNumber(
				asRecord(
					asRecord(model.scoring_sources)[
						apexImputationPolicy.fallbackEvidenceKey
					],
				).score,
			),
...
function prepareImputation(
	...
	for (const key of benchmarkKeys) {
		...
		const imputationPolicy =
			BENCHMARK_CATALOG[key as keyof typeof BENCHMARK_CATALOG]?.scoring
				.imputation;
		if (
			imputationPolicy != null &&
			imputationPolicy.kind !== "contextual" &&
			(imputationPolicy.kind !== "additive_crosswalk" ||
				imputationPolicy.fallback !== "contextual")
		) {
			continue;
		}
