...
function methodologyText(): string {
  return "Model Atlas reports INTELLIGENCE, AGENTIC, SPEED, and VALUE independently and ranks compact public views by INTELLIGENCE. Compact views represent each base model with its highest-INTELLIGENCE scored variant; the all view preserves every scored effort variant. INTELLIGENCE and AGENTIC min-max normalize each selected benchmark against observed values, then average them using benchmark importance multiplied by dimension loading and apply confidence from validation-weighted evidence mass. Each dimension derives its confidence thresholds from the selected portfolio: zero confidence through 10% of total effective weight and full confidence from 60%. Empirical calibration gives each model one total unit of weight across reasoning-effort variants. An unlabelled observation is the source-default variant; when every observation is labelled, source-default selection chooses the highest reported effort. Explicitly labelled observations remain attached to their exact efforts. Model-version replacement activates only when Artificial Analysis benchmark resources and Vals identify the same dated release suffix and the matched route realizes it. Semantic model versions remain distinct identities. The replacement retains values that changed against the prior identity, explicitly identify the new release, carry a newer source observation date no earlier than release, or were already accepted for that replacement. Missing prior data and source reputation alone do not prove freshness. Replacement rows use direct accepted evidence without contextual benchmark imputation, and retained Artificial Analysis and Vals observations receive twice their ordinary effective weight throughout the replacement. Missing values otherwise use one non-recursive, paired-distribution imputation; sibling-effort context is added only when both benchmark and effort evidence are sufficient, at least four held-out models actually use the cross-effort path, and leave-one-model-out error improves by at least 2% or makes a refused one-dimensional predictor reliable. Each effort direction is calibrated separately. A row without enough sibling-effort evidence falls back to the separately validated one-dimensional predictor, penalty, and confidence. Frontier subtracts 1.0x validated error and baseline subtracts 0.5x; cross-only held-out error determines the confidence of a two-dimensional imputation. A configured additive source crosswalk is attempted before contextual imputation and is used only when exact-model-and-effort overlap passes model-family-held-out validation. Imputed values never satisfy public admission. SPEED logs provider inputs before min-max normalization, then equally averages throughput, latency, end-to-end latency, and each active benchmark task-time input. Aggregate price neighborhoods use the linear mean of the public INTELLIGENCE and AGENTIC scores; benchmark time and cost neighborhoods use each benchmark's declared quality coordinate. Logit coordinates are reserved for probability-like completion rates, while linear coordinates preserve native or composite metric spacing. Quality-adjusted price, benchmark time, and benchmark cost subtract the model-excluded expected resource use at comparable quality, average model-balanced percentile and 2.5% one-sided winsorized min-max residual scores, and shrink weakly supported comparisons toward 50. VALUE averages winsorized log blended price, quality-adjusted log blended price, and each active quality-adjusted benchmark task-cost input. Missing benchmark-source task costs and runtimes may use a sibling effort only after at least three directly paired tasks pass leave-one-task-out validation in both log-resource space and the normalized efficiency score. Imputed resources never become calibration anchors, and their influence is weighted by both resource-prediction and benchmark-prediction confidence. SPEED and VALUE apply one coverage multiplier per base-model family using only its existing source-default variant; an unlabelled variant is the default, otherwise the highest reported effort is the default. Every effort receives that same multiplier, and non-default efforts cannot change family coverage. SPEED and VALUE confidence separately report each effort's effective observed and validated evidence share.";
}
