...
export const SNAPSHOT_PRESERVATION_VERSION = 7;
const DEFAULT_EFFORT_KEY = "\u0000default";
const MODEL_EFFORT_SEPARATOR = "\u001f";

function modelKeys(model: ModelAtlasModel): string[] {
  const keys = new Set<string>();
  const effort = canonicalReasoningEffort(model.reasoning_effort) ?? DEFAULT_EFFORT_KEY;
...
function scoreSignalCount(model: ModelAtlasModel, scoringConfig: ScoringConfig): number {
...
function shouldPreservePreviousModel(
  ...
  return (
    ...
    previousIntelligence - currentIntelligence >= policy.minIntelligenceScoreDrop &&
    scoreSignalCount(previous, scoringConfig) > scoreSignalCount(current, scoringConfig)
  );
}

function sortByIntelligence(models: ModelAtlasModel[]): ModelAtlasModel[] {
  return [...models].sort((left, right) => {
    const scoreDelta = right.scores.intelligence_score - left.scores.intelligence_score;
    if (scoreDelta !== 0) {
      return scoreDelta;
    }
...
export function preserveHighSignalSnapshotModels(
  ...
  if (
    previousPayload.metadata.scoring.snapshot_preservation_version !== SNAPSHOT_PRESERVATION_VERSION
  ) {
    return payload;
  }
  ...
  const models = rankedModels(payload.models).map((model) => {
    ...
    if (previous == null || !shouldPreservePreviousModel(model, previous, policy, scoringConfig)) {
      return model;
    }
    replaced = true;
    return previous;
