...
function buildClaudeIdentity(
  ...
  if (!CLAUDE_TIERS.includes(tier as ClaudeTier) || major == null || major.length === 0) {
    return null;
  }
  return {
    tier: tier as ClaudeTier,
    version:
      minor == null ? (COMPACT_CLAUDE_VERSION_ALIASES.get(major) ?? major) : `${major}.${minor}`,
  };
}
...
export function parseClaudeIdentity(value: string): ClaudeIdentity | null {
  ...
  if (versionFirstMatch != null) {
    return buildClaudeIdentity(versionFirstMatch[3], versionFirstMatch[1], versionFirstMatch[2]);
  }
  const tierFirstMatch = normalized.match(TIER_FIRST_PATTERN);
  return tierFirstMatch == null
    ? null
    : buildClaudeIdentity(tierFirstMatch[1], tierFirstMatch[2], tierFirstMatch[3]);
}
