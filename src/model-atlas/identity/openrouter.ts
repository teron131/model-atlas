...
import { claudeRouteIdentityKey } from "./claude";
import {
...
const REASONING_EFFORT_SUFFIXES = REASONING_EFFORT_ROUTES.map(([suffix]) => suffix);
...
function isSameModelVersion(targetModelName: string, candidateModelName: string): boolean {
  ...
  if (candidateBase !== targetBase) {
    ...
    return (
      targetClaudeIdentity != null && targetClaudeIdentity === claudeRouteIdentityKey(candidateBase)
    );
  }
  ...
  return candidateModelName.startsWith(`${targetBase}-`) && isVersionSuffix(suffix);
}

export function isSameOpenRouterModelRoute(targetRoute: string, candidateRoute: string): boolean {
...
export function reasoningEffortSelectionPriority(
  ...
  const normalizedArtificialAnalysisSlug = normalizedEffortRoute(artificialAnalysisSlug);
  const normalizedCanonicalSlug = normalizedEffortRoute(canonicalSlug);
  const exactRoute = normalizedArtificialAnalysisSlug === normalizedCanonicalSlug;
  const routeIndex = exactRoute
    ? -1
    : REASONING_EFFORT_ROUTES.findIndex(
        ([suffix]) => normalizedArtificialAnalysisSlug === `${normalizedCanonicalSlug}${suffix}`,
      );
  const reportedRank =
...
export function isOpenRouterFreeRouteId(modelId: string | null | undefined): boolean {
  return modelId?.endsWith(OPENROUTER_FREE_ROUTE_SUFFIX) === true;
}

export function hasPublicFreeRouteLabel(modelName: string | null | undefined): boolean {
...
export function publicOpenRouterModelName(
  ...
  let publicName = modelName
    .replace(/^Gemini (.+?) Preview(?: \d{2}-\d{2}|\s+\d{2}-\d{4})?$/i, "Gemini $1")
    .replace(/\s+(?:\(latest\)|latest)\s*$/i, "")
    .replace(/\s+\(free\)\s*$/i, "")
    .trim();
  if (hasPlainLatestSuffix && !/\d/.test(publicName)) {
    const version = modelSlugFromModelId(modelId)?.match(/(?:^|-)(\d+(?:\.\d+)+)$/)?.[1];
    if (version != null) {
