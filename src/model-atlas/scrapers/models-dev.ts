...
function isRecentDate(isoDate: string | undefined, cutoffIsoDate: string): boolean {
  if (!isoDate) {
    return false;
  }
...
function extractVercelGatewayModels(html: string): VercelGatewayModelRecord[] {
  ...
  for (const match of matches) {
    ...
    byModelId.set(modelId, {
      ...
      model_id: modelId,
      release_date: isSentinelString(releaseDate) ? null : (releaseDate ?? null),
    });
  }
...
function shouldRetainModel(
  ...
  const matchesRetainedClaudeIdentity = [row.model_id, row.model.name].some((value) => {
    ...
    return (
      identity != null &&
      options.retainedClaudeIdentityKeys?.has(claudeIdentityKey(identity)) === true
    );
  });
  return (
    options.retainedModelIds?.has(normalizeProviderModelId(row.model_id)) === true ||
    (typeof row.model.name === "string" &&
      options.retainedModelNames?.has(normalizeModelToken(row.model.name)) === true) ||
...
function rankRecentModels(
  ...
  return models
    ...
    .filter(
      (row) =>
        isRecentDate(row.model.release_date, cutoffIsoDate) || shouldRetainModel(row, options),
    )
    .sort((left, right) => {
      const leftOutputCost = asFiniteNumber(left.model.cost?.output) ?? Number.POSITIVE_INFINITY;
      const rightOutputCost = asFiniteNumber(right.model.cost?.output) ?? Number.POSITIVE_INFINITY;
      if (leftOutputCost !== rightOutputCost) {
        return leftOutputCost - rightOutputCost;
      }
      return (right.model.release_date ?? "").localeCompare(left.model.release_date ?? "");
    });
}
