...
const DEFAULT_LEADERBOARD_URL = "https://www.mercor.com/apex/apex-agents-leaderboard/";
...
function atlasBaseModel(sourceModel: string): string {
  const withoutConfiguration = sourceModel.replace(/\s+\([^)]*\)\s*$/, "").trim();
  const providerQualified = /^(?:Opus|Sonnet|Fable)\b/.test(withoutConfiguration)
...
function loopPassScore(value: unknown): number | null {
  ...
  const harnessScores: unknown[] = Array.isArray(rawHarnessScores) ? rawHarnessScores : [];
...
function parseResultRow(value: unknown): MercorApexAgentsRow | null {
  ...
  const organization = stringValue(provider.name) ?? stringValue(provider.providerId);
