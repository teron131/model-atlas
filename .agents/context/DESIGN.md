...
The signature offers Phase Ledger and Glacier in the fixed dark presentation. Phase Ledger visualises normalized model parameters. Glacier is the default and uses the accepted ambient video; it does not encode live model parameters or respond to the pointer. It uses a poster for reduced motion or unavailable playback, and pauses offscreen or in a hidden document. Light mode and theme toggles are currently disabled across the dashboard and methodology pages; saved light preferences are not applied. See docs/design/glacier-signature.md for assets, framing, lifecycle and source provenance. Phase Ledger blends provider pigments continuously where model fields overlap rather than assigning hard colour territories. Pointer input produces diffusion, waves, refraction, or shear; it never draws a cursor spotlight.
