...
export function LeaderboardCapture({
  ...
  useEffect(() => {
    if (!captureStageMounted || captureStartedRef.current || captureRef.current == null) {
      return;
    }
  ...
  return (
    <div className={styles.leaderboardCapture} data-capture-exclude>
      <button
        ...
        data-state={state}
        disabled={captureStageMounted || state === "rendering" || rows.length === 0}
        title={label}
                ...
                <tr>
                  {scoreSortableColumns.map((column) => (
                    <th
                      ...
                      data-sort-state={
                        column.key === sortState.key ? sortState.direction : undefined
                      }
                      key={column.key}
                      scope="col"
                    >
