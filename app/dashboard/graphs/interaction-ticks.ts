...
export function interactionXAxisTicks(config: InteractionConfig, domain: [number, number]) {
  if (config.key === "context") {
    return config.ticks.filter((tick) => tick >= domain[0] && tick <= domain[1]);
  }
...
function logStepTicks([low, high]: [number, number], format: (value: number) => string) {
  if (!(low > 0) || !(high > low)) {
    return [];
  }
  ...
  const ticks = Array.from({ length: TARGET_TICK_COUNT }, (_, index) =>
    nearestNiceLogTick(10 ** interpolateLinear(logLow, logHigh, index / (TARGET_TICK_COUNT - 1)), [
      low,
      high,
    ]),
  );
...
function linearAxisTicks([low, high]: [number, number], format: (value: number) => string) {
  if (!(high > low)) {
    return [];
  }
...
function uniqueFormattedTicks(ticks: number[], format: (value: number) => string) {
  const labels = new Set<string>();
  return ticks
    .filter((tick) => Number.isFinite(tick))
    .sort((left, right) => left - right)
    .filter((tick, index, sortedTicks) => index === 0 || tick !== sortedTicks[index - 1])
    .filter((tick) => {
