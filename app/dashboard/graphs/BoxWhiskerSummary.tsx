...
export function BoxWhiskerSummary({
  ...
  const hideMinValue = showDomainEndpoints && minDisplayValue === domainMinValue;
  const hideMaxValue = showDomainEndpoints && maxDisplayValue === domainMaxValue;
  const style = {
  ...
  const observedRangeRatio = Math.abs(distribution.max - distribution.min) / domainSpan;
  ...
  const topLabelClassName = separateTopLabels ? styles.boxWhiskerTopLabelsSeparated : "";
  ...
  useEffect(() => {
    ...
    const measure = () => {
      ...
      const medianProbe = root.querySelector(`.${styles.boxWhiskerMedianProbe}`);
      ...
      if (min != null && medianProbe != null && max != null) {
        ...
        const nextMedianOnTop =
          hasHorizontalOverlap(medianRect, min) || hasHorizontalOverlap(medianRect, max);
        setMedianOnTop((current) => (current === nextMedianOnTop ? current : nextMedianOnTop));
      }
    ...
    >
      ...
      <div
        ...
        aria-label={`${label} distribution from ${formatValue(
          distribution.min,
        )} to ${formatValue(distribution.max)} with median ${formatValue(distribution.median)}`}
        role="img"
      >
        {usesObservedTopLabels ? <span className={styles.boxWhiskerMinLabel}>MIN</span> : null}
        {usesObservedTopLabels || medianOnTop ? (
          <span className={styles.boxWhiskerMedianLabel}>
            {usesObservedTopLabels ? "MED" : medianValue}
          </span>
        ) : null}
        {usesObservedTopLabels ? <span className={styles.boxWhiskerMaxLabel}>MAX</span> : null}
      ...
      >
        {showDomainEndpoints && showDomainEndpointLabels ? (
          <span className={styles.boxWhiskerDomainMinValue}>{domainMinValue}</span>
        ) : null}
        {usesObservedTopLabels ? (
        ...
        {showDomainEndpoints && showDomainEndpointLabels ? (
          <span className={styles.boxWhiskerDomainMaxValue}>{domainMaxValue}</span>
        ) : null}
      </div>
    </div>
  );
...
function shouldSeparateTopLabels({
  ...
  if (staggerTopMedian && nearestObservedLabelRatio < TOP_LABEL_COLLISION_RATIO) {
    return true;
  }
  return false;
}

function hasHorizontalOverlap(referenceRect: DOMRect, element: Element, padding = 4) {
  const rect = element.getBoundingClientRect();
  return referenceRect.left < rect.right + padding && referenceRect.right > rect.left - padding;
}
