...
function pointLabelPenalty(box: LabelBox, point: PointObstacle, padding: number) {
...
export function calloutLabelPlacements({
  ...
  for (const label of orderedLabels) {
    ...
    for (const angle of LABEL_DIRECTIONS) {
      ...
      const textAnchor: TextAnchor = cos < -0.35 ? "end" : cos > 0.35 ? "start" : "middle";
      const preferredSide = label.cx > (bounds.left + bounds.right) / 2 ? "end" : "start";
