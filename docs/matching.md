...
An unlabelled observation is the source-default configuration; when every observation is labelled, the highest reported effort becomes the default. Because saturated confidence can favor sparsely measured efforts, a family's single direct effort result bounds missing sibling scoring proxies by canonical effort priority. Public benchmark fields remain exact-effort observations, and the `all` view retains every scored effort variant.
