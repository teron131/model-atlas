...
import { readFrontierCodeRawCache } from "../src/model-atlas/ingest/benchmark-runtimes/frontier-code";
...
try {
  ...
  try {
    ...
    const rawRows = db.prepare("SELECT * FROM frontier_code_raw_rows ORDER BY row_index").all();
  ...
  assert.equal(payload.metadata.scoring.selected_benchmark_keys.includes("frontier_code"), true);
} finally {
  await removeDatabaseFiles(databasePath);
}
