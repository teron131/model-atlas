...
function deepSWERow(reasoningEffort: string | null, passAt1: number): DeepSWELeaderboardRow {
  return {
    model: "Deep Model",
...
function summary(sourceData: ModelAtlasSourceData) {
  ...
  return {
    artificialAnalysisModelId: sourceData.artificialAnalysis.bySlug.get("example-model")?.model_id,
    modelsDevRows: sourceData.modelsDev.rows.map((row) => [row.model_id, row.provider_id]),
    modelsDevLookupProvider: sourceData.modelsDev.byId.get("shared/model")?.provider_id,
    deepSWEDefault: {
...
const surgeRow = benchmarkObservationRow("chartography", null, "Surge Example Model", 0.64);
...
const cachedSourceData = cachedSourceDataFromSnapshots({
  ...
  artificialAnalysisBenchmarkResourceRows: sourceRows.artificialAnalysisBenchmarkResourceRows,
  modelsDevModels,
