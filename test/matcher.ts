...
const providerPools: PreferredProviderPools = {
  ...
  fallback: [model("fallback", "example/example-medium-3.5", "Example Medium Latest")],
};

const preferredProviderOutput = runMatcher(sourceRows, providerPools, 5, STAGE_CONFIG.matcher);
...
assert.equal(preferredProviderOutput.models[0]?.best_match?.model_id, "example/example-medium-3-5");
assert.equal(
...
const exactFallbackOutput = runMatcher(
  ...
  {
    primary: [model("openrouter", "~anthropic/claude-fable-latest", "Claude Fable Latest")],
...
assert.equal(exactFallbackOutput.models[0]?.best_match?.model_id, "claude-fable-5");
...
const compactClaudeVersionOutput = runMatcher(
  ...
  {
    primary: [model("openrouter", "anthropic/claude-sonnet-3.5", "Claude Sonnet 3.5")],
    fallback: [],
  },
  5,
  STAGE_CONFIG.matcher,
...
const missingClaudeTierOutput = runMatcher(
  [source("claude-3-opus", "Claude 3 Opus")],
  {
    primary: [model("openrouter", "anthropic/claude-3-haiku", "Claude 3 Haiku")],
    fallback: [],
  },
  5,
  STAGE_CONFIG.matcher,
...
assert.deepEqual(
  lowerValidVariantOutput.models[0]?.candidates.map((candidate) => candidate.model_id),
...
assert.equal(
  effortTraceRows.find((row) => row.candidate_model_id === "example/example-3-preview")
    ?.rejection_reason,
  "lower_rank",
...
for (const [sourceSlug, sourceName, candidateId, candidateName] of [
  ...
  ["deepseek-v3-2-0925", "DeepSeek V3.2 0925", "deepseek/deepseek-v3", "DeepSeek V3 0324"],
] as const) {
...
const exactVisionLanguageOutput = runMatcher(
  ...
  {
    primary: [model("openrouter", "qwen/qwen3-vl-30b-a3b-instruct", "Qwen3 VL 30B A3B Instruct")],
    fallback: [],
  },
  5,
  STAGE_CONFIG.matcher,
...
assert.ok(
  unmatchedProDiagnostics != null && unmatchedProDiagnostics.candidates.length > 0,
...
assert.equal(
  unmatchedProTraceRows.every((row) => row.rejection_reason === "variant_conflict"),
  true,
...
const noneEffortObservation = matchedRows.find(
  (row) => row.artificial_analysis_id === "google/example-2-5-flash-non-reasoning",
);
...
assert.equal(
  matchedRows.some((row) => row.artificial_analysis_id === "google/example-3-pro"),
  false,
...
function model(providerId: string, modelId: string, modelName: string): ModelsDevModel {
  return {
...
function modelStatsSourceData(
  ...
  const modelsDevModels = [
    model("openrouter", "google/example-2.5-flash-lite", "Example 2.5 Flash Lite"),
    model("openrouter", "google/example-2.5-flash-image", "Example Image"),
    model("openrouter", "google/example-2.5-flash", "Example 2.5 Flash"),
    model("openrouter", "google/example-3-pro-image-preview", "Example Pro Image"),
  ...
  return {
    ...
    artificialAnalysisBenchmarkResources: {
      ...
      observationLookup: buildArtificialAnalysisResourceLookup(artificialAnalysisResourceRows),
      sourceDefaultLookup: buildArtificialAnalysisSourceDefaultResourceLookup(
    ...
    apexAgents: { rows: [], rowsByModelName: new Map() },
    ...
    bioMysteryBench: { rows: [], rowsByModelName: new Map() },
    blueprintBench: {
    ...
    intphys2: { rows: [], rowsByModelName: new Map() },
    mindcube: { rows: [], rowsByModelName: new Map() },
    ...
    rsiBenchmark: { rows: [], rowsByModelName: new Map() },
    simpleQaVerified: { rows: [], rowsByModelName: new Map() },
    spatialviz: { rows: [], rowsByModelName: new Map() },
...
function asBenchmarks(row: Record<string, unknown> | undefined): Record<string, unknown> {
...
function asScoringSources(
  ...
  return row?.scoring_sources && typeof row.scoring_sources === "object"
    ? (row.scoring_sources as Record<string, ArtificialAnalysisBenchmarkResourceRow>)
    : {};
}
