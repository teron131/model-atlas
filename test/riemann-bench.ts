...
assertDeepEqual(findRiemannBenchScore(["missing", "GPT 5.5"], rowsByModelName), 0.416);

assertDeepEqual(findRiemannBenchScore(["Claude Fable 5"], rowsByModelName), 0.55);
...
try {
  ...
  assert.deepEqual(requestedUrls, [RIEMANN_BENCH_LEADERBOARD_URL, customSourceUrl]);
  assert.equal(
