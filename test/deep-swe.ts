...
const rowsByModelName = buildDeepSWEMap([...rows, row("gemini-3-flash-preview", 0.05)]);
...
assertDeepEqual(
  preferredDeepSWELeaderboardRows([{ ...row("v1-fallback", 0.42), source_version: "v1" }]).map(
    ({ model }) => model,
  ),
  ["v1-fallback"],
);
...
const fallbackRows = await getDeepSWELeaderboardStats({
  urls: ["https://deepswe.datacurve.ai/artifacts/missing.json", DEEP_SWE_V1_1_LEADERBOARD_URL],
});
...
const sourceVersions = new Set(sourceRows.data.map((row) => row.source_version));
...
if (
  preferredSourceRows.length < sourceRows.data.filter((row) => row.source_version === "v1.1").length
) {
...
try {
  ...
  const boundedRows = await getDeepSWERawLeaderboardSourceRows({
    concurrency: 2,
    timeoutMs: 1_000,
    urls: Array.from({ length: 6 }, (_, index) => `http://127.0.0.1:${address.port}/${index}`),
  });
