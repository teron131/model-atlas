      ...
      benchmarkView: astro({
        default: astro({
          metadata: astro({
            ...
            models: astroArray([astro("anthropic/claude-fable-5"), astro("openai/gpt-5.5")]),
          }),
          tasks: astro({
            overall: astro({
...
assertDeepEqual(findValsIndexScore(["missing", "Claude Fable 5"], rowsByModelName), 0.75145);
assertDeepEqual(findValsIndexScore(["openai/gpt-5.5"], rowsByModelName), 0.67951);
assertDeepEqual(findValsIndexScore(["bad/missing-accuracy"], rowsByModelName), null);
