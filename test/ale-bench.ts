...
assert.equal(sourceDefaultRows.find((row) => row.model === "model-c")?.score, 300);
assert.deepEqual(
  sourceDefaultRows.find((row) => row.model === "model-c"),
  {
    ...sakanaRows.find((row) => row.model === "model-c" && row.num_self_refine === 1),
    base_model: "model-c",
...
assert.equal(buildAleBenchCrosswalkStatus(sakanaRows, divergentEpochRows).imputationAllowed, false);
