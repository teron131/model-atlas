...
} from "../src/model-atlas/sources/epoch/weirdml";
import { mergeWeirdMlRows, processWeirdMlCsv } from "../src/model-atlas/sources/weirdml";
...
assert.deepEqual(merged.crosswalk.conflictingEpochModels, ["epsilon_high", "alpha-stale_high"]);
...
assert.equal(
  merged.data.find((row) => row.metadata.epoch_model_version === "delta_high")?.source_url,
  "https://epoch.ai/data/external_benchmarks/weirdml.csv",
);
assert.equal(merged.data.find((row) => row.model === "Epsilon (high)")?.canonical_value, 0.6);
assert.equal(
  merged.data.some((row) => row.metadata.epoch_model_version === "epsilon_high"),
  false,
);

const rejected = mergeWeirdMlRows(primaryRows, [epochRows[0] as WeirdMlEpochRow]);
...
const reorderedClaude = mergeWeirdMlRows([reorderedClaudePrimary], [reorderedClaudeEpoch]);
assert.deepEqual(reorderedClaude.crosswalk.conflictingEpochModels, ["claude-opus-4-20250522_16K"]);
