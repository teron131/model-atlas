...
assert.deepEqual(BENCHMARK_CATALOG.terminal_bench_4.source.inputs, [
  {
    group: "standalone",
    id: "terminal_bench_4",
    roles: ["observation"],
...
assert.deepEqual(
  BENCHMARK_CATALOG.apex_agents.source.inputs.map(({ group, id, roles, evidenceKey, runtime }) => ({
    group,
    id,
    roles,
...
assert.deepEqual(BENCHMARK_CATALOG.terminal_bench_4.processing.aggregation, {
  kind: "custom",
});
...
assert.deepEqual(
  BENCHMARK_CATALOG.deep_swe.presentation.taskMetricColumns.map(({ key, metric }) => ({
    key,
    metric,
  })),
  [
...
assert.equal(
  BENCHMARK_OBSERVATION_BINDINGS.find(({ benchmark }) => benchmark === "frontiermath_tier_4")
    ?.loader.kind,
  "epoch_runs",
);

const benchmarkRawWriterTables = BENCHMARK_RAW_WRITERS.map(({ table }) => table);
assert.equal(
