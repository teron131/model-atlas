...
assert.deepEqual(
  rows.map((row) => [row.model, row.reasoning_effort, row.official_rank, row.official_best_effort]),
  [
...
assert.equal(rows.find((row) => row.base_model === "SWE-1.7")?.score_eligible, false);
