...
try {
  ...
  try {
    ...
    insertBenchmarkRawRows(db, snapshots as unknown as SourceSnapshots, SNAPSHOT_TABLES.ale_bench);
    const rawRows = db.prepare("SELECT * FROM ale_bench_raw_rows ORDER BY row_index").all();
  ...
  assert.equal(payload.metadata.scoring.selected_benchmark_keys.includes("ale_bench"), true);
} finally {
  await removeDatabaseFiles(databasePath);
}
