...
function model(
  ...
  return {
    ...
    benchmarks: artificialAnalysis == null ? {} : { apex_agents: artificialAnalysis },
    scoring_sources: {
...
function mercorValue(candidate: TestModel): number | null {
  return asFiniteNumber(asRecord(asRecord(candidate.scoring_sources).apex_agents_mercor).score);
}
...
assert.ok(Math.abs((crosswalk.diagnostic.medianOffset ?? 0) - 0.015172566371681) < 1e-12);
assert.ok((crosswalk.diagnostic.validationMedianAbsoluteError ?? Infinity) < 0.005);
assert.ok(Math.abs((crosswalk.projectionByItem.get(gpt56Max) ?? 0) - 0.383827433628319) < 1e-12);
...
const apexImputation = buildBenchmarkImputationByModel(models, STAGE_CONFIG.scoring);
assert.ok(
  Math.abs((apexImputation.get(gpt56Max)?.get("apex_agents") ?? 0) - 0.383827433628319) < 1e-12,
);
assert.equal(apexImputation.get(lowMissing)?.get("apex_agents"), 0);
const benchmarkPortfolioWithoutApex = Object.fromEntries(
  Object.entries(STAGE_CONFIG.scoring.benchmarkPortfolio).filter(([key]) => key !== "apex_agents"),
);
const scoringWithoutApex = {
  ...STAGE_CONFIG.scoring,
  intelligenceBenchmarkKeys: STAGE_CONFIG.scoring.intelligenceBenchmarkKeys.filter(
    (key) => key !== "apex_agents",
  ),
...
assert.equal(
  buildBenchmarkImputationByModel(models, scoringWithoutApex).get(gpt56Max)?.has("apex_agents") ??
    false,
  false,
);
